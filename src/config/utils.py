...
def get_device(
    ...
    if (
        ...
        and cfg
        and hydra.core.hydra_config.HydraConfig.get().job.num is not None
    ):
        num_gpus = torch.cuda.device_count()
        gpu_id = hydra.core.hydra_config.HydraConfig.get().job.num % num_gpus
        assigned_device = f"cuda:{gpu_id}"
        if verbose:
            logger.info(
                f"Hydra launcher assigned device: {assigned_device} (Job num: {hydra.core.hydra_config.HydraConfig.get().job.num})"
            )
        return torch.device(assigned_device)
    ...
    try:
        ...
        for gpu_idx in range(num_gpus):
            ...
            if verbose:
                ...
                logger.info(
                    f"GPU {gpu_idx} ({device_name}): Memory {memory_factor:.2%} free, "
...
def _select_best_gpu_pytorch(verbose: bool = False) -> torch.device:
    ...
    for gpu_idx in range(num_gpus):
        ...
        if verbose:
            ...
            logger.info(
                f"GPU {gpu_idx} ({device_name}): Free Memory {free_memory/1024**3:.2f}GB"
            )
