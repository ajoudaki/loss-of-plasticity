...
A  manifold $\mathcal{M}\subset\Theta$ induces LoP if gradient of the loss function is tangent to the manifold at every point on the manifold. That is, $\nabla_\theta\Loss(\theta)\in T_\theta\mathcal{M}$ for all $\theta\in\mathcal{M}$, where $T_\theta\mathcal{M}$ denotes the tangent space of $\mathcal{M}$ at $\theta$. This ensures that once the gradient flow enters $\mathcal{M}$, it remains within $\mathcal{M}$.
    % \mf{the 2nd condition looks like a definition rather than a condition. Perhaps be more assertive and mention something like "the menifold should be stable".} \textbf{Stability type:} 
\end{definition}
...
\begin{remark}
If the conditions in Definition~\ref{def:lop} hold irrespective of the specific data distribution generating the loss $\Loss$, we refer to it as structural LoP, implying it arises from the network architecture and irrespective of data distribution. If the conditions hold for a particular data distribution, it will be a data-dependent LoP. In this paper we are primarily interested in structural LoP, as can persist even if we change the task or data distribution. 
\end{remark}
...
Given the definitions above, we can now move on to formally define the model for our neural network. 
...
f_v \Big(\underbrace{\Sigma_{u: (u,v)\in E}w_{u,v}\,h(u)}_{\text{pre-activation }z(v):=}\Big), &\text{otherwise}.
\end{cases}
\]
Here, $x_v$ is the input value for input node $v$, $f_v$ is the activation function associated with node $v$, $\mathrm{in}(v)$ is the set of nodes with edges towards $v$, and $w(u,v) \in \R$ is the weight parameter associated with the edge from $u$ to $v$. The network output is the vector of activations $h(V_{\text{out}})$. \ff{in(v) is no longer used in the eq, so do we still need the definition? why not use it in the eq?}
\end{definition}
...
\begin{figure}[h!]
    ...
    \caption{Causes of Loss of Plasticity emerging during training.\ff{no mention of fig 1 in the text to explain what it is.}}
...
% \widetilde{W}(t) \in \{\widetilde{W}_0 + \Delta\widetilde{W}(W_t) \mid W_t \text{ evolves according to } G \}.
% \)
% This represents a form of \emph{structural} LoP, as the large network's dynamics are permanently constrained to mimic those of the smaller network, irrespective of the data distribution.
% \end{proposition}
...
\begin{figure}
    \centering
    \includegraphics[width=0.24\linewidth]{paper/images/act__dead_fraction_normalizations_barplot.pdf}
    \includegraphics[width=0.24\linewidth]{paper/images/cnn_act__dead_fraction_normalizations_barplot.pdf}
    \includegraphics[width=0.24\linewidth]{paper/images/resnet_block_dead_fraction_normalizations_barplot.pdf}
    \includegraphics[width=0.24\linewidth]{paper/images/vit_block_saturated_frac_normalizations_barplot.pdf}
    \includegraphics[width=0.24\linewidth]{paper/images/act__dup_fraction_normalizations_barplot.pdf}
    \includegraphics[width=0.24\linewidth]{paper/images/cnn_act__dup_fraction_normalizations_barplot.pdf}
    \includegraphics[width=0.24\linewidth]{paper/images/resnet_block_dup_fraction_normalizations_barplot.pdf}
    \includegraphics[width=0.24\linewidth]{paper/images/vit_block_dup_fraction_normalizations_barplot.pdf}
    \includegraphics[width=0.24\linewidth]{paper/images/act__train_acc_p_normalizations_barplot.pdf}
    \includegraphics[width=0.24\linewidth]{paper/images/cnn_act__train_acc_p_normalizations_barplot.pdf}
    \includegraphics[width=0.24\linewidth]{paper/images/resnet_block_train_acc_p_normalizations_barplot.pdf}
    \includegraphics[width=0.24\linewidth]{paper/images/vit_block_train_acc_p_normalizations_barplot.pdf}
    \caption{Normalization reduces the number of dead units and duplicated units.}
    \label{fig:normalizations-recovering}
\end{figure}

\subsection{Escaping a LoP Manifold}
...
\)
\keivan{I think it is good to mention "according to the taylor expansion principle the above holds", or even give a little more explanation in appendix to make the reviewers work easier. It was not obvious for me first time specially confusing $\theta(0)$ and $\theta_0$} Since $\nabla_\theta \Loss(\theta_0)$ is tangent to $\mathcal{M}$ (Definition~\ref{def:lop}(a)) and $v$ is normal, $v^\top \nabla_\theta \Loss(\theta_0) = 0$. Thus,
\[
...
This corollary explains why techniques involving noise injection or periodic re-initialization of parts of the network, such as the continual backpropagation algorithm proposed by \citep{dohare2024loss} , can sometimes succeed in restoring plasticity. If the manifold is only stable \keivan{Back to definition 2.1 and 2.2 didn't we want to solve the issue when it was in a stable sub manifold? Again confused about definition 2.1 looking at here} (positive curvature in all normal directions, Equation~\ref{eq:stable}), noise alone won't help escape via gradient descent. However, if the manifold is of saddle type (Equation~\ref{eq:saddle}), perturbations along directions of negative curvature can initiate escape trajectories. If the manifold is unstable (Equation~\ref{eq:unstable}), any perturbation will lead to escape. This highlights the importance of the loss landscape's geometry around these manifolds.



\begin{figure}[h!]
    \centering
    \includegraphics[width=0.33\linewidth]{paper/images/CBP-scale-rank.png}
    \caption{(Placeholder figure, not final) Bit Flipping experiment on 5M samples. Low rank structures emerge during training with BP but less with CBP.}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.23\linewidth]{paper/images/CBP-dup_fraction-5.pdf}
    \includegraphics[width=0.23\linewidth]{paper/images/CBP-dup_fraction-10.pdf}
    \includegraphics[width=0.23\linewidth]{paper/images/CBP-dup_fraction-50.pdf}
    \includegraphics[width=0.23\linewidth]{paper/images/CBP-dup_fraction-100.pdf}
    \caption{(Placeholder figure, not final) Bit Flipping experiment on 5M samples. Duplicated structures emerge during training with BP but less with CBP.}
    \label{fig:CBP-scale-rank}
\end{figure}
...
% \begin{itemize}
%     \item \textbf{Emergence of Plasticity Loss Symptoms:} We will train standard neural network architectures (e.g., MLPs, ResNets) on continual learning benchmarks (e.g., sequences of image classification tasks derived from CIFAR-100 or ImageNet, similar to setups in \citep{dohare2024loss}). We will track metrics over extended training periods, specifically measuring:
%         \begin{itemize}
...
%             \item The effective rank of representations in hidden layers (e.g., using participation ratio of singular values of the activation Gram matrix \citep{huh2022lowrank}).
%         \end{itemize}
...
%         \begin{itemize}
%             \item Dropout \citep{srivastava2014dropout}.
%             \item Explicit noise injection into gradients or weights during back-propagation (inspired by Corollary~\ref{cor:perturb} and \citep{dohare2024loss}).
%         \end{itemize}
...
\begin{proof}
    \keivan{Please mention which theorem is getting proved here and in the main paper put a reference to this one} Let $f:\R\to\R$ be square-integrable w.r.t. $\gamma = \mathcal{N}(0,1)$ and not a polynomial. Let $z \sim \mathcal{N}(0, \Sigma)$ with $\Sigma_{ii}=1$ and $|\Sigma_{ij}| < 1$ for $i \neq j$. We want to show $C = \E[f(z)f(z)^\top]$ is full rank.
