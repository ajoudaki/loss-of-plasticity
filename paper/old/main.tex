...
The connection between overparameterization, reduced dimensionality, and learning difficulties has deep roots in the analysis of neural network geometry. Hierarchical models exhibit {singularities}—regions in parameter space where the mapping from parameters to function is not unique (e.g., due to unit duplication or vanishing). Foundational work by \citet{fukumizu2000local} and \citet{amari2006singularities} demonstrated that these singularities cause the Fisher Information Matrix to degenerate, leading to slow learning dynamics (plateaus) as gradient descent is attracted to these regions.

\vspace{-7pt}
\paragraph{Implicit Bias and Stochastic Dynamics.}
Recent work has highlighted how the optimization algorithm itself contributes to the collapse towards simpler representations. \citet{chen2023stochastic} analyzed the implicit bias of Stochastic Gradient Descent (SGD), showing that gradient noise induces an attractive force towards these singular regions (termed "Invariant Sets"). This "Stochastic Collapse" suggests that the tendency towards LoP states is exacerbated by the stochastic nature of the optimization process, even if the regions are unstable under deterministic gradient descent. Furthermore, it is crucial to distinguish between different aspects of LoP; \citet{lee2024s} empirically demonstrated that maintaining {trainability} (ability to fit new data) does not guarantee {generalizability} (performance on unseen data), emphasizing the need for methods that restore genuine plasticity.
...
\GIU{Should we add a remark on SGD? While the full batch gradient could satisfy the manifold condition, the mini batch might not?}

Given these definitions, we can formalize existence of these LoP manifolds, restricting subsequent learning. We present a central proposition that jointly addresses LoP arising from frozen and duplicate units.
...
Both $\mathcal{M}_F$ and $\mathcal{M}_C$ are linear LoP manifolds in the sense of Def.~\ref{def:lop}. Formal proofs and further details are provided in Appendix~\ref{app:cloning_manifold_details}.
\end{proposition}

\vspace{-5pt}
\paragraph{Intuition.}
(i) \emph{Frozen units.} If a unit stays in a regime with \(f'(z_v)=0\) for all finite inputs (e.g., \(\tanh\) with very large \(\lVert\theta_{\mathrm{in}}(v)\rVert\) or ReLU with a large negative bias), then \(\partial \mathcal{L}/\partial \theta_{\mathrm{in}}(v)\approx 0\); its incoming parameters are fixed, so updates are tangent to \(\mathcal{M}_F\).
(ii) \emph{Cloning via redistribution.} Row/column-sum equalities mean total in/out weight is uniformly redistributed within each block pair \((S_i,S_j)\). Collapsing blocks to super-nodes makes forward activations and back-errors identical within blocks; per-edge gradients \(h(u)\,\delta(v)\) are therefore constant on \(S_i\times S_j\) and obey the same equalities. Hence the gradient is tangent to \(\mathcal{M}_C\), and first-order updates remain on both manifolds.



\begin{remark}
It is important to note that the Duplicate Manifold $\mathcal{M}_D$ (defined formally via Incoming and Outgoing Equitable partitions in Appendix~\ref{app:cloning_manifold_details}) represents a significant generalization of the cloning concepts typically discussed in literature. Prior analyses of singularities \citep{fukumizu2000local} or invariant sets \citep{chen2023stochastic} generally define cloned units by requiring their associated weights to be strictly identical (the block-wise constant condition in our terminology). Our framework proves that invariance under gradient descent holds even under the relaxed condition of equitability, where individual weights may differ as long as specific incoming and outgoing sums are maintained. This significantly broadens the class of structures identified as LoP manifolds.
\end{remark}
...
% The only exception is weight decay, as w all might violate the symmetry assumptions that were inherent to the proof of this theorem.
\end{remark}
...
Therefore, a \emph{stable} LoP manifold actively resists escape while an unstable manifold is the easiest to escape. A saddle manifold presents a mixed scenario, where random perturbations may or may not escape depending on the perturbation vector being in a positively or negative space orientation. 
...
\GIU{So, this section is crucial but not clear. In particular, (1) we should introduce the concept of "cloned units" before proposition 3.1, and say that Prop.  3.1 holds for layers with non cloned units.(2) We should give an intuition of this $\alpha$ term with a few examples (it seems to me that it becomes negative for a linear function?). Or we should get rid of $\alpha$ and present the results in a different way. (3) Corollary 3.1 is not clear at the moment.}
...
% \ff{non-positive or $\le 0$? I see this was commented, not sure what I'm missing}
...
The above corollary states that continuously adjusting an activation function, e.g., by shifting or scaling its pre-activations, to maximize its rank recovery strength ($\alpha$), drives the activation into a regime where its derivative is zero for almost all inputs, hence becoming ``frozen'' and stop learning. You can see Section~\ref{app:max_rank_frozen}
%\ff{can we have a more direct reference to the proof? I think this was originally so bc we didn't have the proofs at the time of submission, but we can fix it now} 
%\AJ{The detailed proof turned out to be very involved and long (5-6 pages) and is a bit incomplete, so I rephrased the sentence to we're "discussing" it not proving it. Is that ok, or you prefer to include the actual proof?}
%for a detailed discussion of this corollary. We can get a better intuitive understanding of this statement by going back to the example of Tanh activation.
\ff{I reviewed A.1.1, and left some comments. above comment says the detailed proof is long. do we include it in the appendix anywhere or are we just dropping it? if dropping it, we should change it from "Corollary" to "Conjecture" and provide a brief proof sketch in A.1.1.}
...
\GIU{This is super cool, and we should sharpen out the presentation to really bring the message home. In particular the statement of corollary 3.1 has to be clarified.
Also, for the last paragraph we could draw a small sketch that shows how these LoP symptoms are connected to the network dynamics?}
...
% \ff{may be better as a subsection than a paragraph} 
...
Each task is trained for 500 steps, and validation is performed periodically, resulting in 20,000 total training steps. 
...
%\ff{the text says it is training sequentially on 10 tasks but the plots are showing steps going up to 25000.... ?}
...
\ff{thanks for correcting the text about number of steps. it's odd to me that we count validation steps as part of the total steps. Is the model being updated during validation? If not, can we change the plots so that the total steps is 20k?}
\GIU{For Fartash: the model is not updated during validation, it's a wandb thing, that it updates its internal counter with every call to the server. We updated the plots to show the task rather than the confusing "steps" variable.}
...
\begin{figure}[!ht]
    ...
    \includegraphics[width=0.23\linewidth]{paper/images/vit_block_dup_fraction_plot.pdf}
        \vspace{-.3cm}
...
What if LoP conditions, such as widespread frozen units or extensive feature cloning, have already set in? In such cases, mitigation strategies like normalization, which act proactively, may no longer be sufficient to reverse the state, as indicated by cloning experiments where normalization alone doesn't break perfect, established clones.\GIU{Do we have such experiments?} However, similar to our discussion on manifold stability (Section~\ref{sec:existence_lop_manifold}), injecting noise into the training process can be a viable recovery strategy.
...
\GIU{Added this!}In order to demonstrate the recovery potential of noise injection, we design an experiment where the first half of 5M samples is processed by plain Stochastic Gradient Descent (SGD) and in the second half we switch the learning rule to Continual Backpropagation (CBP). 
Figure~\ref{fig:CBP-scale-rank} clearly shows a reversal in trend when the switch happens: whereas SGD causes the representations' rank to drop and the online training loss to increase, CBP amplifies the features' rank and reduces the online training loss, effectively \emph{recovering plasticity.}
Additionally \ref{fig:CBP-scale-dup} illustrates how aspects like rank and feature duplication are affected by the dimensionality of the model. The disparity between SGD and CBP is only increased by the model size, hinting that the model scale might aggravate the symptoms of LoP. For details see Section~\ref{subsec:experiment_paradigms}.  
\begin{figure}[!ht]
    \centering
    ...
    (Experimental details in Appendix~\ref{app:empirical_evidence_appendix}).}
...
\GIU{I'm not convinced by the above examples. Where is the proof that the effective rank grows with increased $a$ or $b$?}
...
The experiment uses 10,000 samples in 50-dimensional space with intrinsic rank 10. \ff{are these sampled from a normal distribution? if so state the mean and variance} As predicted by our theory, all non-linear activations successfully increase the rank, with varying degrees of recovery. Notably, ReLU and ELU achieve nearly full rank recovery, while bounded activations like Tanh and Sigmoid show more moderate but still significant recovery.
...
\ff{the caption says theoretical and emprical overlap but they only overlap for Tanh. We need to discuss why.}
...
\ff{the y-axis title says $\alpha f$ but it should be $\alpha_f$ (missing subscript). also the middle plots say $|f|$ while it should be $|f^\prime|$}
\ff{similar issues in Fig A.4}
...
\begin{figure}
    ...
    \includegraphics[width=0.33\linewidth]{paper/images/resnet_cloning_losses_plot.pdf}
