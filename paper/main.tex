...
The above corollary is stating that if we continuously adjust an activation function, e.g., by shifting or scaling its pre-activations, to maximize its rank recovery strength ($\alpha$), this process often drives the activation into a regime where its derivative is zero for almost all inputs, hence becoming ``frozen'' and stop learning. You can see Section~\ref{app:max_rank_frozen}
...
\GIU{This is super cool, and we should sharpen out the presentation to really bring the message home. In particular the statement of corollary 3.1 has to be clarified. I think it would be easier to say that 
...
Each task is trained for 500 steps, and validation is performed periodically, resulting in approximately 25,000 total steps shown in the figures. 
...
\ff{yes, I was thinking to drop the preliminary and move all the definitions to their corresponding sections. The remaining paragraph about the model and loss may go into what's currently section 3}
