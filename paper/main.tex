...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.95\textwidth]{rank_.pdf}
    % Placeholder if the image is not yet generated or for draft purposes:
    \fbox{\parbox[c][10cm][c]{0.9\textwidth}{\centering \Large Placeholder for Figure~\ref{fig:eigenvalue_impact_scaling} \\ \vspace{1cm} \normalsize This figure would display the eigenvalue spectra generated by the Python script, showing the output $\Sigma(f(z))$ for various input scaling scenarios compared against the input $\Sigma(z_{base})$.}}
