...
sys.path.insert(0, '.')  # ensure root is in sys.path
...
""" This is a version of the bit flipping experiment where the learned is changed midway through the experiment."""
...
def run_slowly_changing_regression_experiment_with_tracking(cfg: DictConfig) -> dict:
    ...
    for i in tqdm(range(cfg.training.n_examples)):
        ...
        if i > cfg.training.n_examples * cfg.training.switching_point: 
            cfg.training.continual_backprop = True # switching to continual backpropagation after half the training
