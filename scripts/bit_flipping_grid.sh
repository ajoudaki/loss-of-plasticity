...
declare -a HIDDEN_SIZES=(100)

# Switching points to test
declare -a SWITCH=(0.25 0.75)
...
for seed_val in $(seq $START_SEED $END_SEED); do
    for hidden_size in "${HIDDEN_SIZES[@]}"; do
        for switch in "${SWITCH[@]}"; do
            ...
            CMD="$BASE_PYTHON_CMD $N_EXAMPLES_OVERRIDE $CURRENT_SEED_OVERRIDE $DEVICE_OVERRIDE $WANDB_RUN_NAME_OVERRIDE $HIDDEN_SIZE_OVERRIDE $SWITCH_OVERRIDE wandb_tags=[scale]"
            ...
            echo "--------------------------------------------------"
            echo "Launching (Job $((job_count + 1))/$TOTAL_RUNS, Seed: $seed_val, Config: $config_name_suffix, Hidden Size: $hidden_size, Switch: $switch GPU: cuda:$gpu_id):"
            echo "CMD: $CMD"
            echo "--------------------------------------------------"
